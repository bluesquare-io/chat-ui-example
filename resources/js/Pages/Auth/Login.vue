<template>
    <h2 class="text-center text-3xl font-extrabold text-gray-900">{{ ('login.title') }}</h2>
    <p class="mt-2 text-center text-sm text-gray-600">
        {{ ('login.subtitle') }} <Link :href="route('register')" class="font-medium text-primary-600 hover:text-primary-500">{{ ('login.subtitle.link') }}</Link>
    </p>
    <Panel card edge class="mt-8">
        ...
        <form @submit.prevent="submit" class="space-y-4">
            <InputGroup :label="('login.form.email')" target="email" :error="form.errors.email">
                <Input type="email" v-model="form.email" required autofocus autocomplete="username" />
            </InputGroup>

            <InputGroup :label="('login.form.password')" target="password" :error="form.errors.password">
                <Input type="password" v-model="form.password" required autocomplete="current-password" />
            </InputGroup>

            <InputGroup target="remember">
                <Checkbox v-model="form.remember">{{ ('login.form.remember') }}</Checkbox>
            </InputGroup>
